...
func TestAIAgentHandler_Matches_ClientError_Returns500(t *testing.T) {
	h := NewAIAgentHandler(&fakeAIAgentClient{matchesErr: errBoom}, &fakeAttendeeRepo{}, allAIFeaturesOn, nil, nil)
...
func TestAIAgentHandler_Matches_HappyPath_OneEnrichedOneNotFound(t *testing.T) {
	...
	h := NewAIAgentHandler(client, attendees, allAIFeaturesOn, nil, nil)
...
func TestAIAgentHandler_Matches_FeatureDisabled_Returns503(t *testing.T) {
	...
	h := NewAIAgentHandler(client, &fakeAttendeeRepo{}, config.AIFeatureStatus{EnabledMatchMaker: false}, nil, nil)
	r := newAIAgentTestRouter(h, testUser)
