...
func TestAIAgentHandler_Chat_ClientError_Returns500(t *testing.T) {
	h := NewAIAgentHandler(&fakeAIAgentClient{chatErr: errBoom}, &fakeAttendeeRepo{}, allAIFeaturesOn, nil, nil)
	r := newAIAgentTestRouter(h, testUser)
...
func TestAIAgentHandler_Chat_FeatureDisabled_Returns503(t *testing.T) {
	...
	h := NewAIAgentHandler(client, &fakeAttendeeRepo{}, config.AIFeatureStatus{EnabledChatAssistant: false}, nil, nil)
	r := newAIAgentTestRouter(h, testUser)
